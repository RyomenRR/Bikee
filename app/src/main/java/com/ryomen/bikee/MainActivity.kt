...
private fun autoReconnect() {
    ...
    val lastMac = prefs.getString("last_mac", null) // ✅ FIXED key here
...
private fun attemptAutoReconnect() {
    ...
    if (!bluetoothAdapter.isEnabled) {
        val enableBtIntent = Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE)
        startActivityForResult(enableBtIntent, 123)
        return // Wait until user enables it, then reconnect after result
    }
...
override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    ...
    if (requestCode == 123 && resultCode == RESULT_OK) {
        // User enabled Bluetooth, now retry reconnect
        attemptAutoReconnect()
    }
}
